...
def adjust_stock_on_sale(sender, instance, created, **kwargs):
    ...
    if created:
        # 🛡️ Garantizamos atomicidad local para soportar el select_for_update
...
def update_stock_on_received_po(sender, instance, **kwargs):
    ...
    # Verificamos si ya se procesó para evitar duplicidad si se guarda la OC varias veces
    if instance.status == PurchaseOrder.POStatus.RECEIVED:
        with transaction.atomic():
            for item in instance.items.all():
                # select_for_update aquí también para evitar colisiones con ventas simultáneas
